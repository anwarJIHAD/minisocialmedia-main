...
def sign_in():
    ...
    if result:
        payload = {
            "id": username_receive,
            # Token hanya valid selama 24 jam
            "exp": datetime.utcnow() + timedelta(seconds=60 * 60 * 24),
        }
        token = jwt.encode(payload, SECRET_KEY, algorithm="HS256").decode('utf-8')  # Konversi bytes menjadi string
        ...
        return jsonify(
            {
                "result": "fail",
                "msg": "Kami tidak dapat menemukan pengguna dengan kombinasi id/sandi tersebut",
            }
        )
    ...
    try:
        ...
        for post in posts:
            ...
            post["thumbsup_by_me"] = bool(
                db.likes.find_one({"post_id": post["_id"], "type": "thumbsup", "username": payload["id"]}))
